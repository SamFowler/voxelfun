...
public:
    ...
    const Block* addBlock(const BlockPos& position, const BlockMakeType&);
    const Block* addBlock(const BlockPos& block_pos, const std::vector<Voxel>& voxels, const std::vector<Colour>& colours);
    ...
    void updateBlocks(std::vector<const Block*> blocks_to_remesh); //updates blocks batched in m_blocks_to_update and add ptrs to them to remesh vector
...
private:
    ...
    std::vector<unsigned int> m_blocks_to_update;
