...
BlockID BlockManager::addBlock(const Block& block)
{
    ...
    {
        new_block_id = ((--m_blocks.end())->first) + 1 ; //index key of map end + 1
    } 
    else 
    {
    ...
    m_blocks.emplace( new_block_id, block);
    return new_block_id;
}
...
Block BlockManager::getBlock(const BlockID& block_id)
{
